...
public class Assert {
    ...
    //构造paramException
    private static BusinessException createParamException(String msg){
        BusinessException err = new BusinessException(EmBusinessError.PARAMETER_VALIDATION_ERROR);
        err.setErrMsg(msg);
        return err;
    }
    
    public static void isUserExist(boolean expression) throws BusinessException {
        if (!expression) {
    ...
    public static void isTrue(boolean expression, String message) throws BusinessException {
        if (!expression) {
            throw createParamException(message);
        }
    }
    ...
    public static void isInstanceOf(Class<?> type, Object obj, String message) throws BusinessException {
        ...
        if (!type.isInstance(obj)) {
            throw createParamException((StringUtils.hasLength(message) ? message + " " : "") + "Object of class [" + (obj != null ? obj.getClass().getName() : "null") + "] must be an instance of " + type);
        }
    }
    ...
    public static void isAssignable(Class<?> superType, Class<?> subType, String message) throws BusinessException {
        ...
        if (subType == null || !superType.isAssignableFrom(subType)) {
            throw createParamException(message + subType + " is not assignable to " + superType);
        }
    }
